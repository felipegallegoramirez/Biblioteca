...
@Component({
  selector: 'app-inputinsectos',
  templateUrl: './inputinsectos.component.html',
  styleUrls: ['./inputinsectos.component.css']
})
export class InputinsectosComponent implements OnInit {
  id: string = '';
  ...
  comprobar(data:insecto): Boolean{
    /*
    Nombre: 32
  ...
  async enviar(): Promise<void> {
    ...
    if (this.id != '' && this.id ) {
      if(this.insecto.url){
        url=this.insecto.url
      }
      if(this.insecto.imagenid){
        imagenid=this.insecto.imagenid
      }
    ...
    if (this.id != '' && this.id) {
      ...
      if(this.comprobar(envio)){
        response = await this.CentralService.putinsecto(this.id,envio);
